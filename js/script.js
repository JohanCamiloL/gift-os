...
const getSuggestionsGifs = async () => {
  ...
  for (let i = 0; i < 4; i++) {
    const gif = await getRequest(`${GIF_URL}/gifs/random?api_key=${API_KEY}`);
...
const getTrendingGifs = async () => {
  ...
  const gifs = await getRequest(
    `${GIF_URL}/gifs/trending?api_key=${API_KEY}&limit=${limit}`
  );
...
const putSuggestionGifsOnWebPage = async () => {
  ...
  suggGifs.forEach((gif) =>
    createGifSuggestionElement(suggGifs.indexOf(gif) + 1, gif)
  );
};
...
const createGifElement = (gifElement, section) => {
  ...
  const pContent = gifElement.title
    .split(" ")
    .map((word) => `#${word} `)
    .join("")
    .toLowerCase();
...
const createGifSuggestionElement = (position, gifElement) => {
  const divElement = createHtmlElement(
    "div",
    ["gif-info", `column-start-${position}`],
    "",
    ""
  );
  ...
  document
    ...
    .lastElementChild.appendChild(divElement);
};
...
const createGifMenuState = () => {
  ...
  document.getElementById("crear-guifo-window").style.display =
    displayState === "true" ? "grid" : "none";
  document.getElementById("back-arrow").style.display =
    displayState === "true" ? "block" : "none";
};
